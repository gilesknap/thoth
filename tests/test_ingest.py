...
def test_ingest_commit_false_and_deferred_classify_fails(
    harness: IngestHarness,
) -> None:
    """``commit=False`` on the DEFERRED path stages the hold without committing (#80).

    This proves the interaction the ``thoth capture`` batch path relies on when the LLM
    is unavailable: with classify forced to fail (``_RaisingClient``) AND
    ``commit=False``, the inbound item is held durably in ``inbox/`` and the report is
    ``deferred`` but NOT yet committed -- the hold is staged in the working tree for the
    caller's batched commit, not pushed per file. ``_RecordingGitSync`` proves neither
    the per-call orient pull nor the deferred commit ran inside ``ingest``.
    """
    git = _RecordingGitSync(harness.config, env=harness.env)
    llm = LLM(harness.config, client=_RaisingClient())  # force the deferred path
    ingestor = Ingestor(
        harness.config,
        harness.vault,
        llm,
    ...
    report = ingestor.ingest(
        Capture(text="held but not committed", source="import", filename="held.md"),
        commit=False,
        as_is=True,
    )
    ...
    assert report.committed is False
    assert report.conflict is False
    # Neither the per-call orient pull nor the deferred commit ran inside ingest.
    assert git.pull_calls == 0
    assert git.commit_calls == 0
    # The inbox holding is staged in the working tree (durable, awaiting batch commit).
    holds = _inbox_holds(harness)
    assert len(holds) == 1
    assert harness.vault.page_exists(holds[0])
