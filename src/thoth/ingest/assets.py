...
class _AssetStore(_IngestorBase):
    """Stages binaries into ``raw/assets`` under the SHA-256 idempotency rule."""
    ...
    def _obtain_primary_asset(
        ...
        *,
        ...
        A ``url`` capture reuses the analyse pass's single download when present, with
        no second fetch and no leaked temp, falling back to fetching for a standalone
    ...
    def _save_fetched_asset(
        ...
        Idempotent on the fetched bytes' SHA-256. When the destination asset already
        holds byte-identical content the move is skipped as ``'skipped_unchanged'`` and
        the staged tmp file is cleaned up, while a byte mismatch at the same slug
        surfaces as drift and never an overwrite. On the happy path
        :meth:`Vault.save_asset` moves the tmp file, so only the error and skip paths
        must clean it up.
        """
    ...
    def _save_local_asset_result_named(
        ...
        move never consumes the caller's original, the Slack or MCP tmp download, and a
        ...
        bytes-SHA-256 idempotency and drift rule as :meth:`_save_fetched_asset` applies,
        and the staged tmp copy is always cleaned up on the skip and error paths.
        """
    ...
    def _store_text_asset(self, asset_name: str, text: str) -> str | None:
        ...
        written to a fresh tmp file and handed to :meth:`_store_asset`, so the
        bytes-SHA-256 idempotency and drift rule applies and the tmp never leaks.
        Returns the stored vault-relative path, or ``None`` when the best-effort write
        fails.

        Crucially, a derived artifact is an *enhancement* and must never lose nor defer
        the already-durable primary capture. An :class:`IngestError` from
        :meth:`_store_asset` is therefore swallowed to ``None`` here, leaving the
        existing asset untouched rather than aborting the capture (ADR 0009). Drift is
        the realistic case, because
    ...
    def _store_asset(self, tmp_path: Path, asset_name: str) -> RawCaptureResult:
        ...
        *before* the move. Equal bytes mean an idempotent skip, different bytes mean
        drift, a loud error and never a silent overwrite, and a missing asset means a
        fresh create. The staged file is unlinked on every path that does not hand it to
        :meth:`Vault.save_asset`, meaning skip and drift, and on a ``save_asset``
        failure such as a malformed asset filename, so no ``thoth-*`` temp file leaks.

        Raises:
            IngestError: when the staged bytes differ from an existing asset's
                bytes, the drift case, or when the vault rejects the write.
        """
