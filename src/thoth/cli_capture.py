...
def _ingest_one(
    ...
    *,
    ...
    into the next batch. That means a genuine file, with ``page_paths`` non-empty, and
    also an ``unchanged`` skip. ``unchanged`` is reported only when the curated page
    provably already exists (#113), so such a hold duplicates already-filed content. A
    deferred or skipped hold stays, recoverable and idempotent, so a budget re-trip
    never silently deletes un-filed content.
