...
These are pure value types shared by every check module: the :class:`Severity` ordering,
the frozen :class:`Finding` record, the aggregated :class:`LintReport`, the
...
class Severity(IntEnum):
    """Lint finding severity, where a lower value sorts first in the report.

    The order (SPEC check 13) runs broken links and embeds, orphans, source drift,
    contested, stale and overdue, then style.
    """
...
class LintReport:
    ...
    def by_severity(self) -> list[tuple[Severity, list[Finding]]]:
        """Groups findings by severity, most severe first.

        Returns:
            ``(severity, findings)`` pairs, only for severities that occur
        """
    ...
    def render(self) -> str:
        """Renders the report grouped by severity as plain text.

        Each group is a header line ``<SEVERITY> (<count>)`` followed by one indented
        ``check N <name>: <path> -- <message>`` line per finding. A clean report renders
        a single ``lint: clean`` line.
