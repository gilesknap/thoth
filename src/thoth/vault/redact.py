...
# Token-shaped patterns, each conservative enough that ordinary prose and short words
# never match
...
def redact_secrets(text: str) -> str:
    """Replaces secret-looking substrings with a fixed ``[REDACTED]`` marker.

    Masks provider-prefixed API keys, AWS access key ids, ``Bearer <token>`` headers,
    ``key=VALUE`` assignments for a sensitive key set, and long opaque hex or base64
    blobs. The match is conservative, so ordinary prose and short words are left
    untouched. Never raises; a non-string input is returned unchanged.
    """
...
# Writer-controlled structural fields, generated from validated inputs rather than user
# free-text. The sha256 digest is a 64-char hex string the long-hex-blob rule would mask
...
def _redact_frontmatter(meta: dict[str, object]) -> dict[str, object]:
    """Returns a copy of ``meta`` with secrets redacted from its string values.

    Recurses into lists and dicts, and preserves non-string scalars so frontmatter
    typing and date stamping are not disturbed. :data:`_NEVER_REDACT_FIELDS` passes
