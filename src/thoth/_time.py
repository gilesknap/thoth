...
collection: summary, lint, budget and alerts. A consumer that exposes ``LONDON`` as
public surface re-exports it from here.
"""
...
:class:`zoneinfo.ZoneInfo` resolves it. The base dependencies declare ``tzdata``, so it
resolves the same way across the 3.11-3.14 matrix, even on a minimal container with no
operating-system time-zone database.
"""
