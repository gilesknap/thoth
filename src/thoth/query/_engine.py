...
:mod:`thoth.query._retrieval`, :mod:`thoth.query._blend` and
...
class QueryEngine:
    ...
    Every collaborator is injected. The :class:`~thoth.vault.Vault` is real, over the
    ...
    no network I/O itself, delegating recall and prose to the collaborators.
    """
    ...
    def __init__(
        ...
        Args:
            config: The frozen runtime config, kept for parity with the sibling modules,
                since link encoding is delegated to ``vault``.
            vault: The real, path-confined vault facade.
            hindsight: The semantic recall seam, subprocess-backed in production.
            llm: An optional LLM for prose composition. With ``None`` the answer
                falls back to a deterministic excerpt of the top page.
        """
        self._config = config
        self._vault = vault
    ...
    def answer(
        ...
        *,
        ...
        """Blend structural and semantic retrieval by RRF, then compose (#143).

        Two retrieval *sources* both vote on the cited set:

        * the STRUCTURAL source: a grep hit list, grep scanning frontmatter too so a
          page's ``summary:`` gloss matches there, followed by link-graph navigation
          from those hits, deduped and existence-checked into one ordered list;
        * the RECALL source: semantic Hindsight recall, which **always** gets a vote
          when ``use_recall`` is true, with no "only when results are thin" gate.

        Recall is the expensive, subprocess-backed pass, so it is submitted to a worker
        thread FIRST and the cheap structural pass runs on the calling thread while
        recall is in flight. Recall's latency therefore overlaps grep instead of
        serialising after it, because ``subprocess.run`` releases the GIL while it
        waits. The recall worker is pure, returning its path list and mutating no shared
        state, and all the dedup and merge happens single-threaded after the join.

        **Reciprocal Rank Fusion** (:data:`RRF_K`) merges the two ranked lists. Each
        unique path scores ``Σ 1 / (RRF_K + rank)`` over the sources it appears in, and
        paths sort by that fused score descending, structural discovery order breaking
        ties so a structural hit leads a recall hit on a score tie. The top
        ...
        The injected LLM writes the prose when present, and otherwise a deterministic
        excerpt of the top page stands in. Either way the harness builds the citation
        block from confined, real paths. With an LLM the result's citations are the
        **used** subset the model named on its ``USED:`` line (issue #34),
        ...
        message it passes them here, so the grep ranks on those terms rather than the
        raw prose: "list me the docs about dogs" greps ``dog``, not the noise words. The
        recall pass and the composed prose stay keyed off the original ``query``, so the
        answer still reads naturally and the citation and USED logic is unchanged. An
        empty or ``None`` ``search_terms`` falls back to grepping ``query`` verbatim,
        today's behaviour.

        Args:
            query: The natural-language query.
            max_pages: The maximum number of candidate pages to consult and cite.
            use_recall: When false, skips the semantic Hindsight pass entirely,
                spawning no worker thread, for the cheap structural-only path.
            search_terms: Optional lexical keywords from the intent gate, grepped
                instead of the raw ``query``. Empty or ``None`` greps ``query``.

        Returns:
            A :class:`QueryResult` whose citations all resolve to real vault pages.

        Raises:
            QueryError: when no vault page matches the query at all.
        """
        return _answer(
            self._vault,
            self._hindsight,
    ...
    def follow_links(self, path: str, *, limit: int = 20) -> list[str]:
        ...
        Reads the vault-confined page at ``path`` and extracts every link, both the OKF
        standard ``[text](path.md)`` form and any residual ``[[target]]`` wikilink, with
        the alias and ``#anchor`` suffixes stripped. It resolves each target to a real
        page, probing each searched folder for a bare slug, and returns the unique,
        existing targets in body order. A dangling link naming no such page is silently
        skipped, so the result only ever holds real, confined paths.
        ...
        Returns:
            ...
            when ``path`` does not exist or has no resolvable links.
        """
    ...
    def recall_paths(
        ...
        **only those that resolve to a real, confined vault page**. That defends against
        a stale or poisoned index whose ``SOURCE:`` line names a page that no longer
        exists, or a path that would escape the vault: such a hit is dropped rather than
        fabricated into a citation. Order is preserved and duplicates removed.
        ...
        :data:`~thoth.vault.REFERENCE_TYPES`, meaning ``entity``, ``note`` and
        ``memory``, excluding the actionable ``action`` type of todos and the to-consume
        media queue, and keeping the precision it had when only knowledge was indexed.
        With the knowledge and life-admin families gone, the scope is the reference and
        actionable axis carried on the page ``type`` tag rather than a family. A caller
        wanting actionable recall passes a different ``types`` set, or ``None`` to
        search everything.
        ...
        Args:
            ...
            types: The page-type domain scope forwarded to :meth:`Hindsight.recall`,
                defaulting to reference types, where ``None`` searches all indexed
                content.
    ...
    def build_citation(self, path: str) -> Citation:
        ...
        The path runs through :meth:`~thoth.vault.Vault.obsidian_uri`, which first calls
        :meth:`~thoth.vault.Vault.resolve`, so a path outside the vault raises
        ...
        confined path, the ``wikilink`` derives from the real filename stem, and the
        ...
        Raises:
            thoth.vault.PathConfinementError: when ``path`` escapes the vault root.
            thoth.vault.VaultError: when the page does not exist.
        """
