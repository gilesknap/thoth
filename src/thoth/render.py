...
and a daily or weekly digest line. The format is the concise, Slack-native one: a single
clickable ``<obsidian-uri|title>`` link and nothing more. Issue #63 dropped the trailing
vault-relative path. Title-only is the chosen default, because the path is noise next to
a clickable title, and the reader never wants to copy it.
...
:class:`SlackPoster`, the one ``chat.postMessage`` protocol that every Slack output
surface shares.
"""
...
def render_vault_ref(*, obsidian_uri: str, title: str, path: str) -> str:
    ...
    harness-built ``obsidian://`` deep link. The same shape serves a web citation, so
    pass the URL as ``obsidian_uri`` and the page title as ``title``. The
    ``obsidian_uri`` comes verbatim from the caller, and this function never fabricates
    a link.
    ...
    Args:
        ...
        path: The vault-relative path. Issue #63 stopped rendering it as a trailing
            suffix, but it is still the label when ``title`` is blank (issue #67).

    Returns:
        A single ``mrkdwn`` link of the form ``<obsidian_uri|label>``. The ``label`` is
